...
# Translate text from French to English using deep_translator
def translate_column_deep_translator(df, columns):
...
# Translate column names from French to English using deep_translator
def translate_column_names_deep_translator(df):
    translator = GoogleTranslator(source='fr', target='en')
    translated_columns = {}
    for col in df.columns:
        try:
            ...
            print(f"Error translating column name '{col}': {e}")
            translated_columns[col] = col  # Fallback to original column name
    df.columns = [translated_columns[col] for col in df.columns]
    return df
...
def main():
    ...
    df = translate_column_names_deep_translator(df)

    # Drop the 'Horodateur' column
    if 'Timestamp' in df.columns:  # Assuming 'Horodateur' translates to 'Timestamp'
        df = df.drop(columns=['Timestamp'])

    # Translate text from French to English, including 'Plus'
    text_columns = df.select_dtypes(include=['object']).columns
    df = translate_column_deep_translator(df, text_columns)
